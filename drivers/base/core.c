...
// TINNO BEGIN
// Modified by guomingyi,for reboot test block issue, 20160927.	
//#define SHUTDOWN_MONITOR
#ifdef SHUTDOWN_MONITOR 
#include <linux/kthread.h>
extern void force_reboot(void);
...
{    	 
    do {
        mdelay(5000);
        if (sd_counter > 0) {
        	pr_emerg("guomingyi# %s: Oops! force shutdown or reboot ! sd_counter:%d\n", __func__, sd_counter);
		    force_reboot();
		    break;
        }
    } while(1);
...
static void start_monitor_kthread(void)
{
    ...
    if (IS_ERR(sd_thd)) {
        pr_emerg("guomingyi# %s: -%d create fail!\n", __func__, __LINE__);
        sd_thd = NULL;
    }
}
#endif
